...
declare_id!("7EYCDpa5qkNyzKzWj2wFUbQTTuob39jnLCJeymhMRkom");

#[program]
pub mod solana_savings {
    use super::*;
    ...
    pub fn get_balance(ctx: Context<GetBalance>) -> Result<u64> {
        ...
        let user_key = ctx.accounts.user.key();
        match deposit_account.user_deposits.iter().find(|deposit| deposit.user == user_key) {
            Some(deposit) => Ok(deposit.amount),
